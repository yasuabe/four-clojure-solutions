(ns four-clojure-solutions.problem164
...
(checker
 (fn [{:keys [start
              ...
              transitions]}]
   (letfn [(g [stream]
             (if-let [[[cs cur :as pair] & tail] stream]
               (lazy-seq
                (cons
                 pair
                 (g (if-let [edges (transitions cur)]
                      (lazy-cat tail (map
                                      (fn [[c next]] [(cons c cs) next])
                                      edges))
                      tail))))
               nil))]
     (->> (g (list [() start]))
 ...
 (fn [f](= #{"hi" "hey" "hello"}
           (set (f '{:states #{q0 q1 q2 q3 q4 q5 q6 q7}
                     ...
                     :transitions {q0 {h q1}
                                   ...
                                   q5 {l q6}
                                   q6 {o q7}}}))))
 (fn [f](= (set (let [ss "vwxyz"] (for [i ss, j ss, k ss, l ss] (str i j k l))))
 ...
 (fn [f](let [res (take 2000 (f '{:states #{q0 q1}
                                  :alphabet #{0 1}
                                  :start q0
                                  :accepts #{q0}
 ...
 (fn [f](let [res (take 2000 (f '{:states #{q0 q1}
          ...
          (and (every? (partial re-matches #"n*m") res)
               (= res (distinct res)))))
 (fn [f](let [res (take 2000 (f '{:states #{q0 q1 q2 q3 q4 q5 q6 q7 q8 q9}
